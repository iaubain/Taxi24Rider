...
public class Command {
    @Autowired
            RiderProcessor driverProcessor;
    @Autowired
    ...
    public ResponseEntity exec(Map<String,String> headers, String body, Map<String, String[]> params){
        try {
            ...
            switch(command){
                case CREATE_RIDER:
                    return ReturnUtil.isSuccess(new UtilSerializer(Rider.class).deSerialize(driverProcessor.createRiders(body)));
                case EDIT_RIDER:
                    return ReturnUtil.isSuccess(new UtilSerializer(Rider.class).deSerialize(driverProcessor.editRiders(body)));
                case FILTER_RIDER:
                    return ReturnUtil.isSuccess(new UtilSerializer(Rider.class).deSerialize(filterProcessor.filterDriver(body)));
                default:
